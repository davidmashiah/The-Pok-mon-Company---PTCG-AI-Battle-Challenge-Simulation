...
pre_turn = -1
ability_used = False
# Guard against repeatable state-neutral Abilities. _score_ability returns
# 30000 -- the highest score in the policy -- so an Ability re-activatable
# without changing state is selected forever. Measured: 66 of 120 games vs
# one leaderboard deck never terminated (ABILITY -> SWITCH_ENERGY ->
# ATTACH_FROM x333). With actTimeout=0 and a 600 s episode budget that is
# a forfeit, and it is invisible to win rate because stalled games are
# dropped from the sample. Limit 6 bounds the loop without blocking
# legitimate repeated use (limit 2 measurably cost win rate).
ability_uses = {}
...
class AdvancedPolicy:
    ...
    def _score_ability(self, option) -> float:
        ...
        if ability_uses.get((card.id, getattr(card, "serial", None)), 0) >= ABILITY_REPEAT_LIMIT:
            return -1
