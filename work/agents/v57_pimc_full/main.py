...
def _load_deck():
    """Bundle first, cwd second, constant last -- all three agree. The version
    this replaces wrote deck.csv from the DECK constant at import and then read
    that file back, so a bundled decklist could never win (see v34_stadium)."""
...
class AdvancedPolicy:
    ...
    def _score_ability(self, option) -> float:
        ...
        # Pure anti-hang backstop. v14 added limits of 6/turn and 25/game after
        # 66 of 120 games against one leaderboard deck never terminated; those
        # limits also shaped ordinary play. These are set an order of magnitude
        # higher: a normal game never approaches them, an infinite Ability loop
        # still stops.
        _key = (card.id, getattr(card, "serial", None))
        if ability_uses.get(_key, 0) >= 40: return -1
        if ability_uses_game.get(_key, 0) >= 200: return -1
    ...
    def _remember_lunatone_ability(self, ranked: list[int]) -> None:
        ...
        card = get_card(self.obs, option.area, option.index, self.my_index)
        # Feed the anti-hang counters. Without this the limits in _score_ability
        # are declared but never reached, and the backstop is dead code -- the
        # exact silent-no-op failure that has bitten five components here.
        if card is not None:
            _k = (card.id, getattr(card, "serial", None))
...
# remainingOverageTime=600). v51 uses 0.4 s of it and v43 uses 12.9 s. That idle
# 99% is the largest unused resource in this project, and this is what spends it.
#
# Every previous search here stopped early and asked an EVALUATOR to judge the
# position, and every one of them made the agent worse:
#     1-ply + hand-written evaluate()   v30  0.3500 vs v14
#     2-ply + PIMC, same evaluator      v31  0.0530
#     1-ply + learned value net leaf    v33  0.3667 vs v43
# The evaluator was the common factor. A playout that runs to a TERMINAL state
# needs none: the engine reports the winner, which is exactly what we optimise.
# Measured at ~80 ms per complete game, 0 truncated.
import fsearch as _fs

# The harness allows 600 s per EPISODE and v51 uses 0.4 s of it. Everything
# measured locally was capped at 90 s purely so a 60-game gauntlet would finish
# this century -- which means every local reading UNDERSTATES this agent.
#
# Budget scaling is real, not assumed: the same agent at 90 s beats an identical
# copy given 22 s by 0.600 over 60 games. Playout ranking is a Monte Carlo
# estimate, so more samples strictly tightens it.
#
# 280 s keeps two safety margins: under half the 600 s pool (local hardware is
# not Kaggle hardware), and under the gate's own 300 s per-episode assertion.
_PIMC_EPISODE_BUDGET_S = 280.0
_PIMC_PER_MOVE_CAP_S = 9.0
_pimc_spent = 0.0
_pimc_det = None
_pimc_last_turn = -999


def _pimc_reset_if_new_game(obs):
    global _pimc_det, _pimc_spent, _pimc_last_turn
    t = obs.current.turn if obs.current is not None else 0
    if _pimc_det is None or t < _pimc_last_turn:
        _pimc_det = _fs.Determinizer(my_deck)
        _pimc_spent = 0.0
    _pimc_last_turn = t


def _pimc_policy(sobs):
    """Pilot BOTH sides during a playout with the same heuristic."""
    try:
        r = AdvancedPolicy(sobs).choose()
        if r:
            return r
    except Exception:
        pass
    try:
        ...
        return list(range(min(max(1, sobs.select.minCount), n)))
    except Exception:
        return [0]


def _pimc_rank(obs, base_order):
    """Re-rank the heuristic's top few by simulated win rate. Never raises."""
    global _pimc_spent
    try:
        ...
        if obs.select.context != SelectContext.MAIN:
            return None
        # AdvancedPolicy.choose() returns ranked[:maxCount], and maxCount is 1 on
        # MAIN -- so the caller's `ordered` is a ONE-element list and any search
        # fed from it sees a single candidate and declines. That is exactly how
        # best_action was starved in an earlier bundle. Rebuild the FULL ranking
        # here: construct the policy (which sets the global attack plan the way
        # choose() would), then sort every option by its own score.
        _ap = AdvancedPolicy(obs)
        _ap._plan_attack()
        _sc = [_ap._score_option(o) for o in obs.select.option]
        base_order = sorted(range(len(_sc)), key=lambda i: -_sc[i])
        # options the policy has actively rejected are not worth playouts
        base_order = [i for i in base_order if _sc[i] > -1] or base_order
        # AdvancedPolicy.choose() returns ranked[:maxCount], and maxCount is 1
        # on MAIN -- so `base_order` arrives with a SINGLE element and every
        # search in this file has silently starved on it before (HANDOFF gotcha
        # #5, "best_action starved because choose() truncates to maxCount=1").
        # Rebuild the full ranking here. _plan_attack must run first because
        # _score_option reads the plan it sets.
        _ap = AdvancedPolicy(obs)
        _ap._plan_attack()
        _sc = [_ap._score_option(o) for o in obs.select.option]
        base_order = sorted(range(len(_sc)), key=lambda i: -_sc[i])
        # only rank options the heuristic considers legal-ish; a -1 score means
        # "never do this" and simulating it wastes the budget
        base_order = [i for i in base_order if _sc[i] > -1] or base_order
        if len(base_order) < 2:
            return None
        # Spend the budget where games are decided. Most MAIN frames are setup
        # (play a Basic, attach the turn's energy) and the heuristic handles them
        # fine; searching all 72 of them burned 330 s of the 600 s pool for one
        # game. Gate on a frame that offers an ATTACK: that is the decision that
        # takes prizes, and concentrating the playouts there buys far more
        # resolution per second than spreading them thin.
        if not any(o.type == OptionType.ATTACK for o in obs.select.option):
            return None
        _pimc_reset_if_new_game(obs)
        remain = _PIMC_EPISODE_BUDGET_S - _pimc_spent
        if remain <= 5.0:
            return None
        _pimc_det.note_opponent(obs)
        budget = max(0.5, min(_PIMC_PER_MOVE_CAP_S, remain / 12.0))
        # samples scale with the budget; 25 was sized for the 90 s build

        # AdvancedPolicy mutates module globals (plan, ability_used and the
        # ability counters) every time it is constructed. A playout constructs
        # it hundreds of times, so without this the real game would resume with
        # counters inflated by simulated turns and its own attack plan replaced
        # by one from a hypothetical position.
        global plan, ability_used
        _saved = (plan, ability_used, dict(ability_uses), dict(ability_uses_game))
        try:
            t0 = time.time()
            out = _fs.pimc_terminal(obs, _pimc_det, _pimc_policy,
                                    base_order[:3], time_budget=budget,
                                    samples=70, max_candidates=3)
            _pimc_spent += time.time() - t0
        finally:
            plan, ability_used = _saved[0], _saved[1]
            ability_uses.clear(); ability_uses.update(_saved[2])
            ability_uses_game.clear(); ability_uses_game.update(_saved[3])
        return out
    except Exception:
        return None
# ==============================================================
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if pre_turn != obs.current.turn:
        ...
        ability_used = False
        plan = AttackPlan()
        # per-turn counter resets; the per-GAME counter resets only when the
        # turn number goes BACKWARDS, which is the only signal that a new
        # episode has started in this harness
    ...
    try:
        ...
        _ranked = _pimc_rank(obs, ordered)
        if _ranked: ordered = _ranked
