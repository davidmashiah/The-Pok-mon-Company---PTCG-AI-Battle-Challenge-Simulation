...
class AdvancedPolicy:
    ...
    def choose(self) -> list[int]:
        ...
        # Keep the UNTRUNCATED ranking available. choose() returns only
        # select.maxCount entries, which is 1 for MAIN, so a caller wanting to
        # re-rank alternatives would otherwise be handed a single candidate and
        # silently do nothing. Measured: full-turn search returned None on
        # 87/87 calls and spent 0.0s before this was fixed.
        self.full_ranking = list(ranked)
...
_FULLTURN_BUDGET_S = 240.0     # of the 600 s episode pool
_ft_spent = 0.0
...
def _best_line(obs, ordered):
    """Re-rank the heuristic's options by simulating the rest of our turn.

    AdvancedPolicy mutates module globals (plan / ability_used / pre_turn and
    the ability counters) through _plan_attack and _remember_lunatone_ability.
    Running it inside a simulation would corrupt the state backing the REAL
    decision, so snapshot and restore everything it touches.
    """
    global _ft_spent, plan, ability_used, pre_turn, ability_uses
    try:
        if _ft_spent > _FULLTURN_BUDGET_S or len(ordered) < 2:
            return None
        _reset_if_new_game(obs)
        remain = _FULLTURN_BUDGET_S - _ft_spent
        budget = max(0.10, min(3.0, remain / 50.0))
        saved = (plan, ability_used, pre_turn, dict(ability_uses))
        t0 = time.time()
        try:
            out = _fs.best_action(obs, _det, _rollout_policy, ordered,
                                  time_budget=budget, max_candidates=10)
        finally:
            plan, ability_used, pre_turn = saved[0], saved[1], saved[2]
            ability_uses = saved[3]
...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        # SEARCH_ALGO is upstream dead code: its search_begin call raises
        # TypeError on every invocation and is swallowed, and on MAIN frames it
        # returns a ONE-element list, which starved the full-turn search of
        # candidates (measured: 435/435 no-ops). Call the policy once directly
        # -- once, not twice, because choose() increments the Ability counters.
        _pol = AdvancedPolicy(obs)
        ordered = _pol.choose()
        _cands = getattr(_pol, "full_ranking", ordered)
        _re = _best_line(obs, _cands)
        if _re: ordered = _re
        n = len(obs.select.option)
