...
DECK = [
    6, 6, 6, 6, 6, 6, 6, 6,
    6, 6, 6, 6, 6, 673, 673, 674,
    674, 675, 675, 676, 676, 676, 677, 677,
    677, 678, 678, 678, 678, 1121, 1121, 1121,
    1121, 1123, 1123, 1141, 1141, 1141, 1141, 1142,
    1142, 1142, 1142, 1152, 1152, 1152, 1152, 1159,
    1182, 1182, 1213, 1213, 1213, 1213, 1227, 1227,
    1227, 1227, 1229, 1229,
]
...
class C:
    ...
    # cards introduced by the LB#1 list that the upstream policy never saw
    JUDGE, ULTRA_BALL, WALLYS_COMPASSION = 1213, 1121, 1229
...
class AdvancedPolicy:
    ...
    def _score_play(self, option) -> float:
        ...
        # ---- cards in the LB#1 list the upstream policy has no rule for ----
        # Without these they fall through to `return 10000`, which outranks
        # Switch (6000) and Premium Power Pro (5000) -- i.e. the agent would
        # fire Judge the moment it drew it. Judge shuffles BOTH hands down to
        # 4 cards, so playing it on a good hand throws the game away.
        if card.id == C.JUDGE:
            if self.me.deckCount <= LOW_DECK_COUNT: return -1
            opp_hand = self.opponent.handCount
            my_hand = self.me.handCount
            # disruption: only when they are loaded and we are not
            if opp_hand >= 6 and my_hand <= 4: return 3150
            # or as an emergency refill when our hand is genuinely dead
            if my_hand <= 2: return 3040
            return -1
        if card.id == C.ULTRA_BALL:
            # costs 2 cards from hand. Unlike Poke Pad it CAN fetch a Rule Box
            # Pokemon, i.e. it is our only Item that finds Mega Lucario ex.
            if self.me.handCount < 3: return -1
            need_mega = (self.field_counts[C.MEGA_LUCARIO_EX] == 0
                         and self.hand_counts[C.MEGA_LUCARIO_EX] == 0)
            return 5200 if need_mega else 3400
        if card.id == C.WALLYS_COMPASSION:
            # full heal on a Mega ex, but ALL its Energy returns to hand, so it
            # is a tempo loss unless the Mega is genuinely about to be KO'd.
            best = 0.0
            for p in list(self.me.active or []) + list(self.me.bench or []):
                if p is None or not card_table[p.id].megaEx or not p.maxHp:
                    continue
                missing = p.maxHp - p.hp
                if missing >= 150 and len(p.energies) >= 1:
                    best = max(best, missing)
            if best >= 200: return 3300
            if best >= 150: return 2900
            return -1
...
_AGENT_TAG = 'v6_fullturn'  # deck = LB#1 Majkel1337 (1290.7), recovered from public replay 89591044
_spent = 0.0
_det = None
_last_turn = -999
...
def _reset_if_new_game(obs):
    global _det, _spent, _last_turn, _ft_spent
...
def _lethal_line(obs_dict, obs):
    ...
    try:
        ...
        if _spent > _EPISODE_BUDGET_S:
            return None
        # Search is now gated to frames where a lethal is plausible, so it
        # fires on a small minority of turns -- spend much more on each.
        # Reference point: LB#1 (1290.7) uses only 21 s of its 600 s pool and
        # LB#2 only 70 s, measured from replay 89591044. There is a lot of
        # unused compute on this ladder.
        remain = _EPISODE_BUDGET_S - _spent
        budget = max(0.10, min(3.0, remain / 25.0))
...
_FULLTURN_BUDGET_S = 200.0     # of the 600 s episode pool
_ft_spent = 0.0


def _rollout_policy(sobs):
    """Heuristic continuation used inside simulated turns. Must never raise."""
    try:
        return AdvancedPolicy(sobs).choose()
    except Exception:
        try:
            sel = sobs.select
            n = len(sel.option)
            return list(range(min(max(1, sel.minCount), n)))
        except Exception:
            return [0]


def _best_line(obs, ordered):
    """Re-rank the heuristic's top options by simulating the rest of our turn.

    CRITICAL: AdvancedPolicy mutates the module globals `plan`, `ability_used`
    and `pre_turn` via _plan_attack/_remember_lunatone_ability. Running it
    inside a simulation would therefore corrupt the state used for the REAL
    decision. Snapshot and restore around the whole search.
    """
    global _ft_spent, plan, ability_used, pre_turn
    try:
        if _ft_spent > _FULLTURN_BUDGET_S or len(ordered) < 2:
            return None
        _reset_if_new_game(obs)
        remain = _FULLTURN_BUDGET_S - _ft_spent
        budget = max(0.08, min(2.0, remain / 60.0))
        saved = (plan, ability_used, pre_turn)
        t0 = time.time()
        try:
            out = _fs.best_action(obs, _det, _rollout_policy, ordered,
                                  time_budget=budget)
        finally:
            plan, ability_used, pre_turn = saved
            _ft_spent += time.time() - t0
        return out
    except Exception:
        return None
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # Feed the prize tracker on every frame. Deduction is only possible on
    # deck-reveal frames (obs.select.deck is not None), which are NOT the
    # frames where we search -- so observing only at decision time would mean
    # the tracker never learns anything.
    try:
        _reset_if_new_game(obs)
        _det.observe(obs)
    except Exception:
    ...
    try:
        ...
        # full-turn search: let the ENGINE tell us which option actually leads
        # to the better position, instead of trusting the hand-written score
        _re = _best_line(obs, ordered)
        if _re: ordered = _re
        n = len(obs.select.option)
