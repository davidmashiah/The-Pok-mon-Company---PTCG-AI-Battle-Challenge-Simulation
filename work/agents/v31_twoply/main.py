...
class AdvancedPolicy:
    ...
    def choose(self) -> list[int]:
        ...
        # MAIN has maxCount == 1, so the truncated return value gave the
        # full-turn search a single candidate and best_action bailed at
        # `len(cand) < 2` on every call. Keep the whole ranking for the search;
        # the returned value is unchanged, so heuristic play is byte-identical.
...
# ---------------------------------------------------------------------------
# FULL-TURN SEARCH, finally actually wired in.
#
# Measured: v14 spends 7.4 s of the 600 s per-episode pool -- 1.2%. The reason
# is that its only real search is the proven-lethal path, which fires only when
# lethal_plausible() passes. The upstream SEARCH_ALGO above LOOKS like a search
# but its simulate_action calls search_begin with the wrong signature, raises
# every time, and is swallowed by a bare except -- 3 ms of "search" per game.
#
# fsearch.best_action does the real thing: step each candidate through the
# engine, play OUR remaining turn out with the heuristic, and score the end
# position. It has never been evaluated in games because the two bugs that
# starved it (choose() returning one index on MAIN, and the dead SEARCH_ALGO
# occupying this slot) were only found later.
#
# Budget is charged against the SAME pool as the lethal search so the two
# cannot together blow the episode limit.
# ---------------------------------------------------------------------------
FT_MAX_CANDIDATES = 6
FT_HALF_TURNS = 2      # score AFTER the opponent replies, not at our end of turn
FT_SAMPLES = 3         # average over 3 guesses at the hidden cards
_FT_STATS = _fs.SEARCH_STATS


def _rollout(sobs):
    """A legal, properly-counted selection for the simulated position."""
    try:
...
def _fullturn(obs, base_order):
    """Re-rank the heuristic's MAIN options by what the engine says happens."""
    global _spent
    try:
        ...
        if remain <= 5.0:
            return None
        budget = max(0.30, min(4.0, remain / 40.0))
        _det.note_opponent(obs)
        t0 = time.time()
        out = _fs.best_action2(obs, _det, _rollout, base_order,
                               time_budget=budget,
                               max_candidates=FT_MAX_CANDIDATES,
                               half_turns=FT_HALF_TURNS,
                               samples=FT_SAMPLES)
        _spent += time.time() - t0
        return out
    except Exception:
...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        ...
        ordered = _pol.choose()
        _re = _fullturn(obs, getattr(_pol, "full_rank", ordered))
        if _re: ordered = _re
        n = len(obs.select.option)
