...
# HARDENED vs the upstream notebook: this was an unguarded module-level write.
# If cwd is read-only the agent dies at import, exactly like the __file__ bug
# that killed submission 55194301. We ship deck.csv in the tarball anyway, so
# this write is only a convenience — never let it be fatal.
try:
...
class AdvancedPolicy:
    ...
    def choose(self) -> list[int]:
        ...
        # MAIN has maxCount == 1, so the truncated return value gave any search
        # a single candidate and best_action bailed at `len(cand) < 2` on every
        # call. Keep the whole ranking for the search; the returned value is
        # unchanged, so heuristic play is byte-identical.
        self.full_rank = list(ranked)
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    if (target_index == 0
                            and self.hand_counts[C.PREMIUM_POWER_PRO] > 0
                            and card_table[my_pokemon.id].energyType == EnergyType.FIGHTING):
                        damage += 30
...
# ---------------------------------------------------------------------------
# LEARNED VALUE RE-RANKER  (policy proposes -> value re-ranks -> lethal wins)
#
# Both previous designs failed, and both failures are measured on the ladder or
# in hundreds of games:
#   * a learned net REPLACING the policy's pick        -> v23_dz  389.3 ladder
#   * search with the HAND-WRITTEN evaluate() as leaf  -> v30 0.3500, v31 0.0530
#     (deeper lookahead made it WORSE: the signature of a wrong target)
# So the tuned policy still picks the candidate set, the search only reorders
# its own top few, and the leaf is a net trained on real outcomes: game-split
# AUC 0.7986 against a 0.7039 prize-difference baseline. The previous linear
# value net scored 0.7121 -- no better than counting prizes, which is why
# swapping it in would have changed nothing.
#
# The history window is identical for every candidate at a decision, so it adds
# no bias to the ranking; the ordering is driven by the resulting positions.
# ---------------------------------------------------------------------------
VZ_TOPK = 4
_VZ_OK = False
try:
    import dzfeat as _vzf
    import vznp as _vz
    _VZ_OK = _vz.load()
except Exception:
    _VZ_OK = False
...
_VZ_HIST = None
_VZ_STATS = {"leaf_calls": 0, "searches": 0, "reranked": 0}
...
def _vz_reset():
    ...
    if _VZ_OK:
        try:
            _VZ_HIST = _vzf.History()
        except Exception:
            pass


def _vz_observe(obs_dict):
    if not _VZ_OK:
        return
    try:
        if _VZ_HIST is None:
            _vz_reset()
        _VZ_HIST.push(obs_dict)
    except Exception:
        pass


def _vz_leaf(sobs, my_index):
    """Win probability of a simulated end-of-turn position, or None."""
    if not _VZ_OK or _VZ_HIST is None:
        return None
    try:
        sv = _vzf.featurize(sobs, my_index)
        if sv is None:
            return None
        ht, hc = _VZ_HIST.arrays()
        p = _vz.win_prob(sv, ht, hc)
        if p is not None:
            _VZ_STATS["leaf_calls"] += 1
        return p
    except Exception:
        return None
...
def _rollout(sobs):
    """A legal, properly-counted selection for a simulated position."""
    try:
        ordered = AdvancedPolicy(sobs).choose()
    except Exception:
        ordered = None
    try:
        n = len(sobs.select.option)
        lo = max(1, sobs.select.minCount)
        hi = max(min(sobs.select.maxCount, n), min(lo, n))
    except Exception:
        return [0]
    ordered = [i for i in (ordered or []) if 0 <= i < n]
    if not ordered:
        ordered = list(range(n))
    return ordered[:hi] if ordered else list(range(min(lo, n)))


def _value_rerank(obs, base_order):
    """Reorder the policy's own top-k by the learned value of the resulting
    end-of-turn position. Returns None to leave the policy's order alone."""
    global _spent
    if not _VZ_OK:
        return None
    try:
        if not base_order or len(base_order) < 2:
            return None
        _reset_if_new_game(obs)
        remain = _EPISODE_BUDGET_S - _spent
        if remain <= 5.0:
            return None
        budget = max(0.15, min(2.0, remain / 60.0))
        _det.note_opponent(obs)
        t0 = time.time()
        _VZ_STATS["searches"] += 1
        out = _fs.best_action2(obs, _det, _rollout, base_order,
                               time_budget=budget, max_candidates=VZ_TOPK,
                               half_turns=1, samples=1, evaluator=_vz_leaf)
        _spent += time.time() - t0
        if out and out[0] != base_order[0]:
            _VZ_STATS["reranked"] += 1
        return out
    except Exception:
        return None
...
def agent(obs_dict: dict) -> list[int]:
    _vz_observe(obs_dict)      # before any early return: history must not skip frames
    ...
    global pre_turn, ability_used, plan, ability_uses
    ...
    if pre_turn != obs.current.turn:
        ...
        if obs.current.turn < pre_turn_prev:
            ...
            _vz_reset()        # new episode: history must not leak across games
        ability_uses = {}
    ...
    try:
        _pol = AdvancedPolicy(obs)
        ordered = _pol.choose()
        _re = _value_rerank(obs, getattr(_pol, "full_rank", ordered))
        if _re: ordered = _re
        n = len(obs.select.option)
