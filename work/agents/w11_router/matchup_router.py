...
# Which expert handles which detected archetype. The shipped router sent
# 'mirror' to the mirror expert, 'grass' to the tempo expert, and EVERYTHING
# ELSE -- wall, arch, psychic and unknown -- to the baseline, unrouted. That is
# the whole non-Grimmsnarl 70% of the top field handled by a single expert, and
# it is precisely the slice we need to move from 0.72 to 0.92. Instrumenting the
# live agent showed this router overriding 38% of all decisions while the
# coalition it sits beside fired zero times, so this is the layer that actually
# decides games.
_ROUTE={'mirror':'mirror','grass':'tempo','wall':'baseline','arch':'baseline',
        'psychic':'baseline','unknown':'baseline'}
try:
 import json as _json,os as _os
 for _p in ('/kaggle_simulations/agent/route_table.json','route_table.json'):
  if _os.path.exists(_p):
   _ROUTE.update(_json.load(open(_p)));break
except Exception:
 pass

def choose(obs,baseline,mirror,tempo):
 profile=update(obs)
 _pick={'baseline':baseline,'mirror':mirror,'tempo':tempo}
 preferred=_pick.get(_ROUTE.get(profile,'baseline'),baseline)
