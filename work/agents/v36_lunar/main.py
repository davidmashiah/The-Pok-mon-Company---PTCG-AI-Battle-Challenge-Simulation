...
class AdvancedPolicy:
    ...
    def _score_ability(self, option) -> float:
        ...
        if card.id == C.LUNATONE:
            ...
            # Abilities score 30000 and attachments only ~8000, so the ability
            # always resolved FIRST and threw away the very energy we were
            # about to attach. Measured over real games: 13.8% of turns where
            # the engine offered an attachment ended with none made.
            # The precise fix is ordering, not a hand-tuned threshold: only
            # spend energy on cards once this turn's attachment is already
            # made, or when we hold a spare beyond it.
