...
DECK = [
    ...
    6, 673, 673, 674, 674, 675, 675, 675, 675, 676, 676, 676, 677,
    677, 677, 677, 678, 678, 678, 678, 1102, 1102, 1102, 1102, 1123, 1123,
    1141, 1141, 1141, 1141, 1142, 1142, 1142, 1142, 1152, 1152, 1152, 1159, 1192,
    1192, 1192, 1192, 1227, 1227, 1227, 1227, 1252,
]
...
# ---- weights and deck found by work/tools/evolve.py ---------------------
# Hill-climbed against z_roman950 (the LB-950 baseline), w1_alakazam (5th place),
# w2_archaludon, AND the reigning champion itself at weight 0.40. That last term
# is what removes the ceiling: without it the strongest sparring partner we can
# download is a 950 agent, so the search could never aim higher than 950.
# Nothing was adopted on a screen alone -- each change also had to win an
# 800-game confirmation run on disjoint seeds, which rejected roughly 8 of every
# 10 candidates that screened better.
# Fitness 0.4760 -> 0.5120 over 27 rounds; vs the LB-950 agent 0.505 -> 0.58.
# ---- tunable policy weights -------------------------------------------
# These were hand-chosen magic numbers. Every earlier attempt to tune them
# scored candidates against OUR OWN policy, which is the measurement flaw
# that invalidated this whole project's local metrics. work/tools/evolve.py
# now scores them against a panel of REAL published agents instead.
P = {
    "ABILITY": 32000,
    "PLAY_MON": 20000,
    "ITEM_DEFAULT": 10000,
...
def target_score(pokemon: Pokemon) -> int:
    data = card_table[pokemon.id]
    score = prize_count(pokemon) * P['PRIZE_W'] + len(pokemon.energies) * 300 + len(pokemon.tools) * 200
...
class AdvancedPolicy:
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        score = P['ENERGY_BASE'] + (10 if active else 0)
    ...
    def _score_option(self, option) -> float:
        ...
        if option.type == OptionType.RETREAT: return P['RETREAT'] if plan.attacker >= 1 else -1
        if option.type == OptionType.ATTACK:
            return P['ATTACK_MATCH'] if (option.attackId == MEGA_BRAVE) == (plan.attack_index == 1) else 1000
        return 0
    ...
    def _score_play(self, option) -> float:
        ...
        if data.cardType == CardType.POKEMON:
            ...
            return P['PLAY_MON']
        if card.id == C.SWITCH: return P['SWITCH'] if plan.attacker > 0 else -1
        if card.id == C.PREMIUM_POWER_PRO:
            ...
            return P['PPP']
        if card.id == C.BOSS_ORDERS: return P['BOSS'] if plan.target >= 1 else -1
        if card.id == C.CARMINE: 
            ...
            return -1 if self.me.deckCount <= LOW_DECK_COUNT else P['CARMINE']
        if card.id == C.LILLIE_DETERMINATION: 
            return -1 if self.me.deckCount <= LOW_DECK_COUNT else P['LILLIE']
        if card.id == C.GRAVITY_MOUNTAIN: return P['GRAVITY'] if any(p is not None and card_table[p.id].stage2 for p in self._opponent_board()) else (1200 if self.stadium_id else -1)
        return P['ITEM_DEFAULT']
    ...
    def _score_evolve(self, option) -> float:
        ...
        return P['EVOLVE'] + len(pokemon.energies)
