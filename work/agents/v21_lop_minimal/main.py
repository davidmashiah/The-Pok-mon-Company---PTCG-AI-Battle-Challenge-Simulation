...
# ================= MINIMAL MEGA LOPUNNY EX ROTATION OVERRIDE =================
# Gale Thrust [C] does 60, or 230 if this Pokemon moved from the Bench to the
# Active Spot THIS TURN. Air Balloon (-CC retreat) makes Lopunny's retreat free,
# so with TWO energised Lopunny, retreating one PROMOTES the other and it hits
# for 230 -- every turn, off one Colourless energy.
#
# Only two decisions are overridden:
#   1. retreat when an armed Lopunny is benched  (that IS the combo)
#   2. promote an armed Lopunny whenever asked who becomes Active
# Everything else stays with the tuned policy, which is what actually wins games.
_LOP, _BUN = 849, 848
_AIR_BALLOON = 1174
...
def _lop_armed(p):
    return p is not None and p.id == _LOP and len(p.energies or []) >= 1


def _lop_bench_idx(me):
...
def _lop_override(obs):
    try:
        ...
        # (2) asked who becomes Active -> take an armed Lopunny
        if ctx in (int(SelectContext.TO_ACTIVE), int(SelectContext.SWITCH)):
            bench = me.bench or []
            for i, o in enumerate(opts):
                idx = o.index
                if (o.area is not None and int(o.area) == int(AreaType.BENCH)
                        and isinstance(idx, int) and idx < len(bench)
                        and _lop_armed(bench[idx])):
                    return [i]
            return None

        if ctx != int(SelectContext.MAIN) or len(opts) < 2:
            return None

        act = me.active[0] if (me.active and me.active[0]) else None
        bl = _lop_bench_idx(me)

        # (1) an armed Lopunny is waiting on the bench -> retreat to promote it
        if bl >= 0:
            for i, o in enumerate(opts):
                if int(o.type) == int(OptionType.RETREAT):
                    return [i]

        # if Lopunny is Active without the bonus, prefer Spiky Hopper's 160
        # over Gale Thrust's 60 when it is affordable
        if act is not None and act.id == _LOP and len(act.energies or []) >= 2:
            gale = spiky = None
            for i, o in enumerate(opts):
                if int(o.type) == int(OptionType.ATTACK):
                    if o.attackId == _SPIKY:
                        spiky = i
                    elif o.attackId == _GALE:
                        gale = i
            if spiky is not None and gale is not None:
                return [spiky]
        return None
    except Exception:
        return None
...
def agent(obs_dict: dict) -> list:
    try:
        ...
        if obs.select is not None:
            ...
            if r:
                sd = obs.select
                if sd.minCount <= len(r) <= sd.maxCount:
                    return r
    except Exception:
        pass
    return _INNER_LOP["fn"](obs_dict)
