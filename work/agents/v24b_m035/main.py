...
# ---------------------------------------------------------------------------
# Learned re-ranker (DouZero-style). numpy-only: torch is 518 MB against a
# ~197 MB tarball. dzfeat is the SAME module the offline extractor used, so the
# features here cannot drift from the ones the net was trained on.
# Everything below is fail-open: if any of it breaks, _DZ_PICK returns None and
# the tuned policy -- the thing actually worth ~700 rating -- keeps its choice.
# ---------------------------------------------------------------------------
_DZ_OK = False
try:
...
DZ_MARGIN = 0.35        # logits of separation required before overruling
_DZ_HIST = None
...
def _dz_observe(obs_dict):
    """Feed this frame's logs. Must run on EVERY invocation: the extractor
    pushed logs on every ACTIVE frame, and the agent is called on exactly those."""
    if not _DZ_OK:
        return
    try:
        if _DZ_HIST is None:
...
def _dz_pick(obs_dict):
    """Model's preferred option index, or None to defer to the policy."""
    ...
    try:
        ...
        _DZ_STATS["calls"] += 1
        p = _dznp.rank(sv, af, ac, mk, ht, hc, n, margin=DZ_MARGIN)
        if p is not None:
            _DZ_STATS["fired"] += 1
        return p
    except Exception:
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if pre_turn != obs.current.turn:
        ...
        if obs.current.turn < pre_turn_prev:
            ...
            _dz_reset()            # new episode: history must not leak across games
        ability_uses = {}
    ...
    try:
        ...
        # Learned re-rank: only when the net is clearly confident (DZ_MARGIN).
        # Proven lethal above already returned, so we can never overrule a win.
        _p = _dz_pick(obs_dict)
        if _p is not None and 0 <= _p < n:
            if not ordered or ordered[0] != _p:
                _DZ_STATS["changed"] += 1
            ordered = [_p] + [i for i in ordered if i != _p]
