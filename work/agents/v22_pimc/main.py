...
def _reset_if_new_game(obs):
    global _det, _spent, _last_turn, _pimc_spent
...
# ============ DETERMINIZED MONTE CARLO ON PIVOTAL DECISIONS ONLY ============
# Scored by ACTUAL WINS from playing lines to the end -- never by a hand-written
# evaluation, which is what made the earlier full-turn search 11.2 points worse.
# Fires on roughly the ~10 decisions per game that can turn it, and declines to
# answer unless the win-rate gap clears 2 standard errors. The tuned policy
# keeps every decision this refuses.
import pimc as _pimc

_PIMC_BUDGET_S = 320.0        # of the 600 s episode allowance
_pimc_spent = 0.0


def _pimc_rollout_policy(sobs):
    """Both sides play the tuned heuristic during a playout."""
    try:
...
def _pimc_pick(obs, ordered):
    global _pimc_spent, plan, ability_used, pre_turn, ability_uses
    try:
        if _pimc_spent > _PIMC_BUDGET_S or len(ordered) < 2:
            return None
        _reset_if_new_game(obs)
        if not _pimc.is_pivotal(obs):
            return None
        remain = _PIMC_BUDGET_S - _pimc_spent
        budget = max(1.0, min(12.0, remain / 8.0))
        # AdvancedPolicy mutates plan / ability counters; a playout must not
        # corrupt the state backing the real decision.
        saved = (plan, ability_used, pre_turn, dict(ability_uses))
        t0 = time.time()
        try:
            out = _pimc.choose(obs, _det, _pimc_rollout_policy, ordered,
                               time_budget=budget)
        finally:
...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        ...
        ordered = _pol.choose()
        _full = getattr(_pol, "full_ranking", ordered)
        _mc = _pimc_pick(obs, _full)
        if _mc is not None:
            ordered = [_mc] + [i for i in ordered if i != _mc]
        n = len(obs.select.option)
