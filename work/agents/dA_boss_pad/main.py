...
# 30000 -- the highest score in the policy -- so an Ability that can be
# activated again without changing the game state is selected forever.
# Measured: 18 of 40 games against one leaderboard deck never terminated;
# the loop was ABILITY -> SWITCH_ENERGY -> ATTACH_FROM repeated 333 times.
# On the ladder that exhausts the 600 s per-episode budget, i.e. a loss.
# Upstream patched this for Lunatone only (_remember_lunatone_ability).
ability_uses = {}
ABILITY_REPEAT_LIMIT = 6
# Second, distinct stall: both agents loop ABILITY -> TO_DECK -> END, one
# activation per turn, so the per-TURN limit above never trips. Observed
# running to turn 1312 with neither side attacking; the ability returns a
# card to the deck and the draw takes it back, so the deck never depletes
# and the game cannot end. Bound total activations per card per GAME too.
ability_uses_game = {}
ABILITY_GAME_LIMIT = 25   # 2 was too strict: it blocked legitimate repeated
                          # Ability use and cost win rate. 6 still bounds the
                          # loop at ~18 steps instead of ~999.
