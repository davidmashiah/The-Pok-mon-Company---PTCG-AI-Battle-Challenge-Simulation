...
class C:
    ...
    # measured worst matchups (see FINDINGS.md sec.4)
    FEZANDIPITI_EX = 140          # Cruel Arrow: 100 dmg to ANY Pokemon
    TEAL_OGERPON_EX = 96          # Myriad Leaf Shower: +30 per Energy on BOTH actives
    TEAL_OGERPON_A, TEAL_OGERPON_B = 95, 349
...
class AdvancedPolicy:
    ...
    # Fezandipiti ex snipes our 80 HP Riolu off the bench with Cruel Arrow
    # (100 damage to ANY Pokemon), denying Mega Lucario ex before it exists.
    # Measured: 0.667 win rate vs these decks despite Fezandipiti being weak
    # to Fighting -- we lose the engine, not the damage race.
    def _opponent_snipes_bench(self) -> bool: return self._opponent_has({C.FEZANDIPITI_EX})
    # Teal Mask Ogerpon ex's attack scales +30 for each Energy on BOTH active
    # Pokemon, so over-loading our own attacker feeds their damage.
    # Measured: our two worst matchups overall (0.592 / 0.658).
    def _opponent_is_ogerpon(self) -> bool: return self._opponent_has({C.TEAL_OGERPON_EX, C.TEAL_OGERPON_A, C.TEAL_OGERPON_B})
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id in {C.MAKUHITA, C.HARIYAMA}:
            ...
            # Mega Brave only needs FF. A third Energy adds nothing for us but
            # gives Myriad Leaf Shower +30, so stop at 2 against Ogerpon.
            if self._opponent_is_ogerpon() and energy_count >= 2 and active:
                score -= 900
        return score
    ...
    def _score_active_choice(self, option, card: Pokemon | Card) -> float:
        ...
        # Do not park a 3-prize Mega ex in the Active Spot when it cannot
        # attack. Evidence: across 9 replays of the rank-1 agent (1275.3), 9 of
        # 12 promotion disagreements were them promoting a cheap 1-prize body
        # (Makuhita / Lunatone / Solrock / Hariyama) where we promoted Mega
        # Lucario ex. Knocking out a Mega ex gives the opponent 3 prizes; a
        # Makuhita gives 1. Mega Brave needs FF, so <2 Energy means it is
        # standing there purely as a 3-prize target.
    ...
    def _score_attach(self, option) -> float:
        ...
        if card.id == C.HERO_CAPE:
            score = 7000
            # +100 HP takes Riolu from 80 to 180, out of one-shot range of
            # Cruel Arrow's 100. That protects the evolution line that the
            # whole deck depends on.
            if self._opponent_snipes_bench() and pokemon.id == C.RIOLU: return 13000
            if self._opponent_is_water_deck(): return 12200 if pokemon.id == C.RIOLU else 12800 if pokemon.id == C.MEGA_LUCARIO_EX else score
            # Cape goes on RIOLU, not on Mega Lucario ex.
            # Evidence: replay 89591044, the rank-1 agent (1275.3) attaches
            # Hero's Cape to Riolu. Mechanically it dominates -- Tools persist
            # through evolution, so caping the fragile 80 HP link both protects
            # the evolution line AND still ends up on a 440 HP attacker.
            # The upstream policy preferred Mega Lucario ex (+200 vs +100),
            # which buys only the second half.
            if pokemon.id == C.RIOLU: score += 300
            elif pokemon.id == C.MEGA_LUCARIO_EX: score += 120
            return score
    ...
    def _score_evolve(self, option) -> float:
        pokemon = get_card(self.obs, option.inPlayArea, option.inPlayIndex, self.my_index)
        if not isinstance(pokemon, Pokemon): return 0
        # a Riolu left on the bench against a sniper is a dead Mega Lucario
        if pokemon.id == C.RIOLU and self._opponent_snipes_bench(): return 15000
