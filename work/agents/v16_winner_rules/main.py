...
class AdvancedPolicy:
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id in {C.MAKUHITA, C.HARIYAMA}:
            ...
            # Mined from 45 same-archetype WINNERS in the official top-episode
            # dataset (ctx 21 ATTACH_FROM): winners put Energy on Riolu (23),
            # Lunatone (19), Makuhita (17), Solrock (14), Mega Lucario ex (10).
            # We put it on Makuhita (74) and Hariyama (34). We were feeding the
            # Fighting line instead of the Lucario line the deck wins with.
            else: score -= 60 if energy_count >= 1 else 0
        elif pokemon.id == C.LUNATONE: score += 40 if energy_count < 1 else -100
        elif pokemon.id == C.SOLROCK: score += 60 if energy_count < 1 else -100
    ...
    def _score_option(self, option) -> float:
        ...
        # ctx 43 ACTIVATE: the upstream rule answers YES to every yes/no
        # question (YES->1, NO->0). Winners decline far more often; agreement
        # here was 16.3%. Default to declining optional activations unless the
        # context is one where yes is clearly right.
        if option.type == OptionType.YES:
            if self.context == SelectContext.IS_FIRST: return 100
            if self.context in (SelectContext.MULLIGAN, SelectContext.COIN_HEAD,
                                SelectContext.FIRST_EFFECT): return 1
            return -1 if self.context == SelectContext.ACTIVATE else 1
        if option.type == OptionType.NO:
            return 1 if self.context == SelectContext.ACTIVATE else 0
    ...
    def _score_card_choice(self, option) -> float:
        ...
        if self.context == SelectContext.TO_HAND:
            ...
            # ctx 7 TO_HAND: winners fetch Solrock 27 times where we fetch
            # Lunatone / Riolu / Makuhita.
            elif card.id == C.SOLROCK: score += -250 if self.field_counts[card.id] >= 1 else 130
    ...
    def _score_active_choice(self, option, card: Pokemon | Card) -> float:
        ...
        # ctx 3 SWITCH, same-archetype winners: 51 cases of promoting a cheap
        # 1-prize body (Solrock 15, Lunatone 13, Makuhita 6, Hariyama 4) where
        # we promote Mega Lucario ex. A knocked-out Mega ex gives the opponent
        # 3 prizes; a Solrock gives 1. Tested earlier as v10b and rejected on a
        # local metric whose opponents were piloted by our own policy -- the
        # winners say that metric was wrong.
        if len(card.energies) < 2:
            score -= 45 * prize_count(card)
