...
class AttackPlan:
    def __init__(self, attacker=-1, target=-1, attack_index=-1, remain_hp=-1, needs_energy=False):
        ...
        # remain_hp_now assumes we play none more.
        self.remain_hp_now = remain_hp

plan = AttackPlan()
# Premium Power Pro's buff lasts the whole turn but the card leaves the hand the
# instant it is played, and nothing in State exposes item effects -- so read it
# off obs.logs. It is a COUNT because the buff STACKS: measured in-engine,
# damage to their Active by copies live is 270 / 300 / 330 for 0 / 1 / 2 copies.
# 330 >= Grimmsnarl ex's full 320 HP. Three copies still read 330, so cap at two:
# an overestimate invents knockouts, the one error mode worse than the
# underestimate being fixed.
PPP_MAX_STACK = 2
    ...
    try:
        ...
        for lg in (obs.logs or []):
            ...
            if t == int(LogType.TURN_START) or t == int(LogType.TURN_END):
                ppp_played_this_turn = 0
            elif (t == int(LogType.PLAY) and lg.cardId == 1141
...
class AdvancedPolicy:
    def __init__(self, obs: Observation):
        ...
        # Pokemon do 30 more damage to your opponent's Active". It is an Item,
        # so once played hand_counts drops to 0 and any "is it in hand" test
        # silently stops applying the buff for the rest of the turn -- while the
        # effect is still live. Measured over 60 games: live on 393 of our MAIN
        # frames, unmodelled on 302 of them, 32 real knockouts scored as "no KO".
        ...
        # Gravity Mountain is -30 HP on every Stage 2 in play, so a copy still in
        # hand makes the target read 30 HP too high. Only counted when the engine
        # is actually offering the Stadium play this turn, so it can never invent
        # a knockout.
    ...
    def _score_play(self, option) -> float:
        ...
        if card.id == C.PREMIUM_POWER_PRO:
            # Skip a copy only when the knockout already holds WITHOUT it.
...
def agent(obs_dict: dict) -> list[int]:
    ...
    global pre_turn, ability_used, plan, ppp_played_this_turn
