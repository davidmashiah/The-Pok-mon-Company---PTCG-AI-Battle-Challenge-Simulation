...
# The bundle's own hand-written policy for THIS deck, used to play our side of
# a rollout. All of its per-episode state lives in one module-level dict with an
# explicit reset, which is what makes it safe to borrow: we snapshot that dict
# around every search so a rollout can never corrupt the live game's tracking.
try:
    from policies.handwritten_v26 import manual_policy as _MP
except Exception:
    _MP = None
...
MIRROR_MAIN_ONLY = _i("PTCG_SEARCH_MIRROR_MAIN", 0)
...
OPP_POLICY_MIRROR = _i("PTCG_SEARCH_OPP_POLICY", 0)
# Weight on "whose attacker is about to die". 0 disables the term entirely,
# which is the A/B control.
THREAT_W = _f("PTCG_SEARCH_THREAT", 0.0)
...
def _prizes_for(mon):
    c = _CARD.get(getattr(mon, "id", None))
    if c is None:
        return 1
    return 3 if getattr(c, "megaEx", False) else (2 if getattr(c, "ex", False)
                                                  else 1)


def _best_attack_damage(attacker, defender):
    """Most damage `attacker` can pay for against `defender`, with Weakness."""
    ac = _CARD.get(getattr(attacker, "id", None))
    dc = _CARD.get(getattr(defender, "id", None))
    if ac is None:
        return 0.0
    have = len(attacker.energies or [])
    best = 0.0
    for aid in (getattr(ac, "attacks", None) or []):
        a = _ATK.get(aid)
        if a is None or not a.damage:
            continue
        if len(a.energies) > have:
            continue                      # cannot pay for it
        d = float(a.damage)
        if (dc is not None and getattr(dc, "weakness", None) is not None
                and int(dc.weakness) == int(ac.energyType)):
            d *= 2.0
        if d > best:
            best = d
    return best
...
def _lethal_threat(o, me_idx):
    """Who is about to lose their attacker?

    The leaf could see damage but not DANGER: our Grimmsnarl at 140/320 and one
    at 140/320 that dies to the next Shadow Bullet scored identically, even
    though one of them is about to hand over two prizes. In a prize race that
    distinction is the position. Valued at the prizes actually at stake --
    a Mega ex is worth three, an ex two -- and discounted, because a threat is
    not a certainty: they may not attack, we may retreat or knock them out first.
    """
    st = o.current
    ...
    if ma is None or oa is None:
        return 0.0
    val = 0.0
    if _best_attack_damage(oa, ma) >= float(ma.hp):
        val -= 1000.0 * _prizes_for(ma) * THREAT_W
    if _best_attack_damage(ma, oa) >= float(oa.hp):
        val += 1000.0 * _prizes_for(oa) * THREAT_W
    return val
...
def _policy_pick(o):
    """Play OUR side of the rollout with the bundle's real hand-written policy.

    The rollout used to play both sides with a crude greedy, so every candidate
    was scored under continuations that would never happen. Making that greedy
    merely knockout-aware was most of w34's +55; this removes the rest of the
    error on the side that matters, because our own continuation is what decides
    what the candidate actually achieves.

    Costs 0.14 ms per call plus 0.44 ms to convert the observation -- about
    0.59 ms a step, against a 600 s episode allowance.

    The OPPONENT stays on the greedy deliberately: manual_policy is written
    against our own card ids, so casting it as a Mega Lopunny pilot would be
    worse than a generic knockout-aware one.
    """
    if _MP is None:
        return None
    try:
        sel = _MP.agent(_dc.asdict(o))
        opts = o.select.option or []
        lo = int(o.select.minCount or 0)
        hi = int(o.select.maxCount or 0)
        if (isinstance(sel, list) and sel and len(set(sel)) == len(sel)
                and all(isinstance(i, int) and 0 <= i < len(opts) for i in sel)
                and lo <= len(sel) <= max(hi, lo)):
            return list(sel)
    except Exception:
        pass
    return None
...
def validate(obs_dict, chosen, my_deck):
    ...
    # Borrowing the real policy for rollouts means it accumulates turn history
    # from imagined games. Snapshot its state and put it back afterwards, so the
    # LIVE game's tracking is bit-for-bit what it would have been.
    ...
    try:
        ...
        if _saved is not None:
            ...
            _MP._RUNTIME_PLAYERS.update(_saved)
    try:
        search_end()
    except Exception:
        pass
