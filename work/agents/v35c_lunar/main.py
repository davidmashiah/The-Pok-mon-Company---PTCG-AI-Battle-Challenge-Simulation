...
class AdvancedPolicy:
    ...
    def _score_ability(self, option) -> float:
        ...
        if card.id == C.LUNATONE:
            # Lunar Cycle DISCARDS a Basic {F} Energy from hand to draw 3.
            # That is the same resource our attacks run on (Mega Brave needs
            # two). Measured against a 1265-rated pilot on 2170 of their own
            # positions, we fired this ability 365 times where they fired it
            # 48 -- 7.6x -- and our attach rate was 2.9% of decisions against
            # their 12.0%. We were converting our attack fuel into cards.
            if self.me.deckCount <= LOW_DECK_COUNT: return -1
            # never spend the last energy: keep one to attach
            if self.hand_counts[C.BASIC_FIGHTING_ENERGY] < 3: return -1
            # if the attacker still needs an attachment this turn to reach its
            # attack, the energy is worth more on the board than as 3 cards
            if plan.needs_energy and not self.state.energyAttached: return -1
            # drawing 3 into an already full hand is the weakest case for it
            if len(self.me.hand) > 4: return -1
        return 30000
