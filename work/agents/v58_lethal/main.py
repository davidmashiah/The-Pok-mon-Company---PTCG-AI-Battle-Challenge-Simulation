...
# Cheap (<=0.6 s/move) and it only ever returns a line the ENGINE itself played
# out to a win under our determinization -- it can never invent a win, only miss
# one. An earlier ablation found this worthless (v46 0.495 vs v43), but that ran
# against the OLD opponent-deck library, which had zero Archaludon and zero
# Cinderace lists and therefore determinized those opponents as filler. A
# "proven" win against a deck that does not exist proves nothing. Re-testing now
# that the library is rebuilt from the decks we actually face.
