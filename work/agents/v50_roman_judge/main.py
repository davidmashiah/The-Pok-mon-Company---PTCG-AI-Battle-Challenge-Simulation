...
class AdvancedPolicy:
    ...
    def _score_play(self, option) -> float:
        ...
        if card.id == C.JUDGE:
            # Alakazam's Powerful Hand places 2 damage counters on our Active
            # for EACH CARD IN THEIR HAND -- 20 damage a card, as counters, so
            # Weakness and our 340 HP Mega are irrelevant. Measured on our own
            # ladder replays: Alakazam is 18.8% of our games at a 0.357 win rate
            # and in 19 of 28 their hand alone was lethal. Judge caps it at 4
            # cards. Scored explicitly because the fallback at the end of this
            # method returns 10000 for unrecognised cards, which would dump our
            # hand every turn. Outside that matchup it sits BELOW Carmine (3000)
            # and Lillie's (3100): four copies scored above them cost 0.38
            # head-to-head, two copies scored below them cost nothing.
            if self.me.deckCount <= LOW_DECK_COUNT: return -1
            if self._opponent_scales_with_hand() and self.opponent.handCount >= 6: return 4200
            if len(self.me.hand) <= 3: return 2950
            return -1
