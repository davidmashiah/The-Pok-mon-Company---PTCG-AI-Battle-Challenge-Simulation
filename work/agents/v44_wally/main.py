...
DECK = [
    ...
    1142, 1142, 1142, 1152, 1152, 1159, 1182, 1182, 1182, 1192, 1192, 1192, 1192,
...
class AdvancedPolicy:
    ...
    def _score_play(self, option) -> float:
        ...
        if card.id == C.WALLYS_COMPASSION:
            # "Heal all damage from 1 of your Mega Evolution Pokemon ex. If you
            # healed any damage in this way, put all Energy attached to that
            # Pokemon into your hand."
            #
            # This attacks the structural problem behind every bad matchup: our
            # attacker gives up 3 prizes and dies to exactly two hits, while the
            # decks beating us give up 2 and hit every turn (Archaludon 220,
            # Grimmsnarl 180). Healing a 340 HP Mega Lucario back to full makes
            # them spend four attacks instead of two for the same 3 prizes.
            #
            # The Energy goes to HAND, not the discard, so the cost is one
            # re-attachment rather than the board -- but the Pokemon cannot
            # attack the turn we heal, which is why a knockout available RIGHT
            # NOW always outranks it.
            if plan.attacker >= 0 and plan.remain_hp <= 0: return -1
            worst = 0
            for p in self._my_board():
                if p is None: continue
                if not card_table[p.id].megaEx: continue
                worst = max(worst, p.maxHp - p.hp)
            # below 200 damage they still need two more hits, so healing is not
            # yet buying a turn and the supporter slot is worth more as a draw
            if worst < 200: return -1
            return 4300
        if card.id == C.JUDGE:
