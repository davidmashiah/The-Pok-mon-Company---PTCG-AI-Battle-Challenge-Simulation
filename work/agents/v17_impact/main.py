...
class AdvancedPolicy:
    ...
    def _score_play(self, option) -> float:
        ...
        if data.cardType == CardType.POKEMON:
            ...
            return 20000
        # Measured over 45 same-archetype WINNER games: they play Switch 31
        # times, we play it ZERO. The gate (plan.attacker > 0) opens on 10.1%
        # of MAIN frames, but 6000 sits below Ability (30000), playing a
        # Pokemon (20000), Evolve (9000) and Attach (8000) -- so even when the
        # plan says "switch", something routine always outranks it and the card
        # is never played. Re-priced ONLY inside the frames where the plan
        # already judged it correct, so behaviour cannot change elsewhere.
        if card.id == C.SWITCH: return 21000 if plan.attacker > 0 else -1
        if card.id == C.PREMIUM_POWER_PRO:
            ...
            return 5000
        # Same defect, worse: winners play Boss's Orders 20 times in 45 games,
        # we play it ZERO, with 3 copies in the deck. Dragging the right target
        # into the Active Spot is how prizes get taken; at 3200 it lost to every
        # routine play.
        if card.id == C.BOSS_ORDERS: return 21500 if plan.target >= 1 else -1
