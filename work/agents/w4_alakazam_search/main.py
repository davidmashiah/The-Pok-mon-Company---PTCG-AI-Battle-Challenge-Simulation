...
# ===================== POLICY-AGNOSTIC ABILITY STALL GUARD =====================
# Measured on this agent unmodified: 15 non-terminating games out of 775 across
# the 31-deck library (98.06% completion). A stalled game is not a slow game --
# cabt.json sets actTimeout=0 with a 600 s per-episode budget, so the loop eats
# the whole allowance and forfeits.
#
# This agent guards specific cards by name (ability_used_dudunsparce,
# ability_used_fezandipiti), which is the same narrow patch the other public
# agent used for Lunatone. Any repeatable state-neutral Ability outside that
# list loops forever.
#
# Implemented as a WRAPPER rather than an edit to the scoring logic: it needs no
# understanding of this policy's internals, and it only ever intervenes after an
# Ability has been activated far more times than any legitimate line requires.
# Hold the original inside a NON-callable container. Binding it to a plain
# module-level name (_inner_agent = agent) makes it the last *callable* in the
# module dict -- reassigning `agent` keeps that key at its original position --
# so kaggle_environments' get_last_callable would pick the UNGUARDED original
# and this wrapper would silently never run. Measured: the guard appeared to
# work (12 stalls instead of 15) while never actually executing.
_INNER = {"fn": agent}
_ab_turn = {}
_ab_game = {}
_ab_prev_turn = -10**9
ABILITY_TURN_LIMIT = 6
ABILITY_GAME_LIMIT = 25


def agent(obs_dict: dict) -> list[int]:
    global _ab_turn, _ab_game, _ab_prev_turn
    sel = _INNER["fn"](obs_dict)
    try:
        obs = to_observation_class(obs_dict)
        if obs.select is None or obs.current is None or not sel:
            return sel
        turn = obs.current.turn
        if turn != _ab_prev_turn:
            if turn < _ab_prev_turn:      # new game
                _ab_game.clear()
            _ab_turn = {}
            _ab_prev_turn = turn
        ...
        if i >= len(opts):
            return sel
        o = opts[i]
        if int(o.type) != int(OptionType.ABILITY):
            return sel
        key = (o.area, o.index)
        nt = _ab_turn.get(key, 0)
        ng = _ab_game.get(key, 0)
        if nt >= ABILITY_TURN_LIMIT or ng >= ABILITY_GAME_LIMIT:
            # Veto and take the most progress-making legal alternative.
            for pref in (OptionType.ATTACK, OptionType.END):
                for j, x in enumerate(opts):
                    if int(x.type) == int(pref):
                        return [j]
            for j, x in enumerate(opts):
                if int(x.type) != int(OptionType.ABILITY):
                    return [j]
            return sel
        _ab_turn[key] = nt + 1
        _ab_game[key] = ng + 1
...
# ===================== PROVEN-LETHAL FORWARD SEARCH =====================
# Ladder-measured worth ~+100 on our own policy (v3 701.3 vs v2 606.3, same
# policy otherwise). It is policy-agnostic: find_lethal searches the ENGINE for
# a line ending in result == our index, and only returns one the engine itself
# played out to a win. It can therefore only override the host policy when it
# has proof, which is why it is safe to graft.
...
_LETHAL_BUDGET_S = 240.0
_ls_spent = 0.0
_ls_det = None
_ls_prev_turn = -10**9
_INNER2 = {"fn": agent}
# `del` before redefining: rebinding an existing name keeps its ORIGINAL dict
# position, so any function defined afterwards becomes the last callable and
# kaggle_environments calls THAT instead. Deleting the key first means the new
# `agent` below is inserted fresh at the end of the dict, where the harness
# looks. Verified after this edit: last callable is "agent".
del agent


def _lethal_or_none(obs_dict):
    global _ls_spent, _ls_det, _ls_prev_turn
    if not _FS_OK:
        return None
    try:
        obs = to_observation_class(obs_dict)
        if obs.select is None or obs.current is None:
            return None
        turn = obs.current.turn
        if _ls_det is None or turn < _ls_prev_turn:
            _ls_det = _fs.Determinizer(list(my_deck))
            _ls_spent = 0.0
        _ls_prev_turn = turn
        _ls_det.observe(obs)
        if _ls_spent > _LETHAL_BUDGET_S:
            return None
        _ls_det.note_opponent(obs)
        remain = _LETHAL_BUDGET_S - _ls_spent
        budget = max(0.10, min(3.0, remain / 25.0))
        t0 = _time.time()
        line = _fs.find_lethal(obs, _ls_det, time_budget=budget)
        _ls_spent += _time.time() - t0
        if not line:
            return None
        n = len(obs.select.option)
        line = [i for i in line if 0 <= i < n]
        if line and obs.select.minCount <= len(line) <= obs.select.maxCount:
            return line
        return None
    except Exception:
        return None
...
def agent(obs_dict: dict) -> list[int]:
    line = _lethal_or_none(obs_dict)
    if line:
        return line
    return _INNER2["fn"](obs_dict)
