...
class AdvancedPolicy:
    ...
    def _score_ability(self, option) -> float:
        ...
        #
        # Abilities scored a flat 30000 -- above every other option -- so they
        # always resolved FIRST, before the turn's energy attachment (~8000) and
        # before evolving (9000). Lunatone's Lunar Cycle costs "discard a Basic
        # {F} Energy card from your hand", which is the same resource Mega Brave
        # runs on, so the highest-priority action in the policy was routinely
        # spending the energy we were about to attach.
        #
        # Against 2,170 identical positions the 1265-rated pilot took ATTACH on
        # 41.9% of offered decisions to our 15.0%, EVOLVE 22.5% to our 14.5%,
        # and ABILITY 12.5% to our 41.8%. Ranking abilities BELOW the attachment
        # reproduces that ordering directly, instead of bolting a condition onto
        # the ability (v35/v36 did that and it did not survive measurement).
        return 7500
